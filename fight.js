...
function showMN(min, max)
{
	...
	document.write("<table border=0 cellpadding=0 cellspacing=0 width=260 height=10 bgcolor=#dedede><tr><td width=100% nowrap style='position: relative;display:block;' background='img/icon/grey.jpg'><span style='position: absolute; right: 5; z-index: 1; font-weight: bold;font-size: 10; color: #ffffff;'>"+min+"/"+max+"</span><img src="+color+" alt='Уровень маны' height='10' width="+m2+"%></td></tr></table>");
}
...
function DrawTrick(can_use, img,  txt, free_cast, dsc, resource, select_target, target, target_login, magic_type, name)
{
  ...
  if (can_use)
  {
  	if (select_target)
  	{
	  	s += "<A style='cursor: pointer;' onclick=\"findlogin('</b>Выберите" +
  	...
  	{
		s += free_cast ? '<A HREF="/battle.php?special=' + name + '&r=' + rnd + '">': "<A style='cursor: pointer;' onclick=\"b_confirm('battle.php', '" + txt + "', '" + magic_type + "', '<INPUT type=hidden name=special value=" + name+ ">', 1)\">";
	}
  }

  s +=  '<IMG style="' + (can_use? 'cursor:pointer': "filter:gray(), Alpha(Opacity='70');") + '" width=40 height=25 '+ "src='../i/misc/icons/" + img+ ".gif'";
  if (txt){
...
function print_priems(can_use, id, priem_name, select_target,req, title, description)
{
	...
	{	
		if (select_target==1)
		{
	  		s += "<A style='cursor: pointer;' onclick=\"findlogin('</b>Выберите врага для приема <b nobr nowrap>" + title + "</b>', 'battle.php?special=" + priem_name + "&r=" + rnd + "', 'target',  '', '1')\">";
  		} 
  		else 
  		{
	...
	s += '<IMG style="' + (can_use? "cursor:pointer;": "opacity:0.3; filter:alpha(opacity=30);-moz-opacity: 0.3;") + '" src="img/priem/misc/' + id+ '.gif" border=0';
