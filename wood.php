...
<?php
	...
	else if ($_GET["act"]=="expired_flower" && $login=="Adminion") 
	{
		mysql_query("DELETE FROM `inv` WHERE object_type='flower' and term<".time());
		echo mysql_affected_rows();
	} 
	else if ($_GET["act"]=="reports" && $login=="Adminion") 
	{
	...
	else if ($_GET["act"]=="del_perevod" && $login=="Adminion") 
	{
	...
	else if ($_GET["act"]=="del_bloked" && $login=="Adminion") 
	{
	...
	else if ($_GET["act"]=="del_users" && $login=="Adminion") 
	{
	...
	*/
	...
	//mysql_query("INSERT INTO `inv` (`owner`, `object_id`, `object_type`, `object_razdel` ,`iznos`,  `iznos_max`) VALUES 	('Adminion', '9','runa','runa','0','1');");
	...
	*/
	...
	{
		mysql_query("INSERT INTO inv(owner,object_id,object_type,object_razdel,gift,wear,gift_author) VALUES('".$users["login"]."','27','medal','medal','1','0','UFC')");
	}
	...
	while ($res=mysql_fetch_Array($k))
	{
		...
		mysql_query("INSERT INTO pochta(user,whom,text,subject) VALUES ('Satan','".$res['login']."','".$txt."','Администрация UFC')");
