...
if (!in_array($_SESSION["login"],$dostup))
{
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td align="right">&nbsp;<INPUT type=button style="width:10%; cursor:hand" onClick="location.href='main.php?act=no'" value="Вернуться" class="newbut"></td>
  </tr>
</table>
<a href="?type=add">Добавление вещей в магазин</a> | <a href="?type=edit&otdel=sword">Редактирование вещей</a> | <a href="?type=old">OLD THINGS</a> <? if(in_array($_SESSION["login"],$dostup2)){?>| <a href="?type=all_things">ВСЁ</a>  | <a href="?type=priem">Приёмы</a> | <a href="?type=runa">Кристальный зал</a> | <a href="?type=resurs">Ресурсы</a> | <a href="?type=eliks">Зелья</a> | <a href="?type=medal">MEDAL</a> | <a href="?type=flower">Магазин подарков</a><? }?><hr>
<?
if (isset($_POST["name"]) && !isset($_GET["edit"]))
{
	...
	{
		$msg="Вы не можете загрузить картинки1...";
	}	
}
//--------------------------------------------------------------------
if ($_GET["type"]=="priem")
{	
	...
	if ($_GET["edit"])
	{
		if ($_POST["priem_id"] && $_SESSION["login"]=="Adminion")
		{
...
{	
	...
	if ($_GET["edit"])
	{
		if ($_POST["runa_id"] && $_SESSION["login"]=="Adminion")
		{
		...
		<tr>
			...
			<form action='' method='post'>
					...
					Интуиция: + <input type=text name='add_udacha' value="<?=$runa['add_udacha']?>" class=new size=30><br>	
...
if ($_GET["type"]=="resurs")
{
if ($_POST["aadsv"] && in_array($_SESSION["login"],$dostup))
		{
			mysql_query("INSERT INTO `wood` (`name`,`img`,`price`,`mass`,`lvl`)VALUES('".$_POST["names"]."','".$_POST["img"]."','".$_POST["price"]."','".$_POST["mass"]."','".$_POST["lvl"]."')");
			print"Готово";
		}
if ($_GET["ops"] && in_array($_SESSION["login"],$dostup))
		{
			mysql_query("INSERT INTO inv (owner,object_type,object_id) VALUES ('".$_SESSION["login"]."','wood','".$_GET["ops"]."');");
			print"Вы взяли ресурс";
		}
	...
	{
		if(($i % 15) == 0) echo "</tr>";
		echo "<td><img src='img/".$DAT["img"]."' title='".$DAT["name"]." [".$DAT["id"]."]' onclick=\"document.location='?type=resurs&ops=$DAT[id]'\" style='cursor:hand;'><br>
<img src='/img/icon/plus.gif' onclick=\"document.location='?type=resurs&edit=$DAT[id]'\" style='cursor:hand;'>
</td>";
		$i++;
	}
	echo '</tr></table>';
	if ($_GET["edit"])
	{
		...
		if ($_POST["resurs_id"] && in_array($_SESSION["login"],$dostup))
		{
			mysql_query("UPDATE wood SET name='".$_POST["name"]."', img='".$_POST["img"]."', price='".$_POST["price"]."',mass='".$_POST["mass"]."',lvl='".$_POST["lvl"]."' WHERE id=".(int)$_POST["resurs_id"]);
			print"Готово";
		}
		$resurs_id=$_GET["edit"];
		...
		<tr>
			...
			<form action='' method='post'>
					...
					Масса: <input type=text name='mass' value="<?=$resurs['mass']?>" class=new size=30><br>
					Лвл: <input type=text name='lvl' value="<?=$resurs['lvl']?>" class=new size=30><br><br>
					<input type=submit value=" Изменит " class=new>
			</form>
			</td>
		</tr>
...
?>	
<form action="" method="post">
<input name="cc" type="submit" value="Создать">
</form>
...
<table border=0>
		<tr>
			...
			<form action='' method='post'>
					...
					Названия: <input type=text name='names' value="" class=new size=50><br>
					IMG: <input type=text name='img' value="wood/" class=new size=30><br>
					Цена: <input type=text name='price' value="" class=new size=30><br>
					Масса: <input type=text name='mass' value="" class=new size=30><br>
					Лвл: <input type=text name='lvl' value="" class=new size=30><br><br>
					<input type=submit value=" Создать " class=new name=aadsv>
			</form>
			</td>
		</tr>
		</table>
...
?>
<form action="" method="post">
<input name="id" type="text" value="" size="10"> <input name="koll" type="text" size="10"> <input name="go" type="submit" value="Добавить" class="newbut">
</form>
<?
if(isset($_POST['go'])){
$iik = mysql_query("UPDATE scroll SET  mountown='".$_POST['koll']."' WHERE id='".$_POST['id']."'");
if($iik){
print"<font style='font-size:12px; color:#990000;'>Вы поставили ".$_POST['koll']." шт.</font>";
}else{print"<font style='font-size:12px; color:#990000;'>Ошибка!</font>";}
}

if(isset($_GET['ids'])){
$iik = mysql_query("UPDATE scroll SET  mountown='0' WHERE id='".$_GET['ids']."'");
if($iik){
print"<font style='font-size:12px; color:#990000;'>Вы удачно обнулили шт. в магазине!</font>";
}else{print"<font style='font-size:12px; color:#990000;'>Ошибка!</font>";}
}

if(isset($_GET['idp'])){
$iik = mysql_query("UPDATE scroll SET  mountown=mountown+10000 WHERE id='".$_GET['idp']."'");
if($iik){
print"<font style='font-size:12px; color:#990000;'>Вы удачно добавили 10000 шт. в магазин!</font>";
}else{print"<font style='font-size:12px; color:#990000;'>Ошибка!</font>";}
}
if(isset($_GET['vz'])){
$iik = mysql_query("SELECT * FROM scroll WHERE id='".$_GET['vz']."'");
if($iiw=mysql_fetch_array($iik)){
$del_time= time() + $iiw["del_time"]*24*3600;
mysql_query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$_SESSION["login"]."', '".$iiw['id']."', '".$iiw['type']."', 'magic', '1', '', '0', '".$iiw['iznos_max']."','".$del_time."');");
print"<font style='font-size:12px; color:#990000;'>Вы удачно взяли предмет!</font>";
}else{print"<font style='font-size:12px; color:#990000;'>Ошибка!</font>";}
}
	...
	while ($dat = mysql_fetch_array($seek))
	{
		...
		echo "<TR  bgcolor=".($n?'#D5D5D5':'#C7C7C7').">
		<TD valign=center align=center width=150>
			<img src='img/".$dat["img"]."'><br>[ID:".$dat["id"]."]<br>
<a href='?type=eliks&vz=".$dat["id"]."'>Взять</a>
		</td>
		...
		echo "<b>Цена: ".$dat["price"]."</font>".($dat["art"]?" Пл.":" Зл.")."</b> <small>(количество: ".$dat["mountown"].") <a href='?type=eliks&ids=".$dat["id"]."'><img src='/img/minus.gif'></a> <a href='?type=eliks&idp=".$dat["id"]."'><img src='/img/pluss.gif'></a>  </small><BR>
...
if ($_GET["type"]=="edit")
{
	...
	{
		$query=mysql_query("SELECT * FROM paltar WHERE id=".(int)$_GET["item_id"]);
		...
		if ($_POST["change_img"])
		{
			...
			{
				$msg="Вы не можете загрузить картинки2...";
			}	
		}
		...
		echo "<td><font color=RED><b>Путь к рисункуs: </b></font><INPUT type='file' name='img' id='img' class='new'></td>";
		echo "</tr>";
	...
	else if ($_GET["edit_id"])
	{
		...
		<tr><td>Мин. Интуиция:</td><td><input type=text name="min_udacha" value="<?=$res['min_udacha'];?>" class=new size=30></td></tr>
		...
		<tr><td>+ Интуиция:</td><td><input type=text name="add_udacha" value="<?=$res['add_udacha'];?>" class=new size=30></td></tr>
...
<tr><td>Мин. Интуиция:</td><td><input type=text name="min_udacha" class=new size=30></td></tr>
...
<tr><td>+ Интуиция:</td><td><input type=text name="add_udacha" class=new size=30></td></tr>
