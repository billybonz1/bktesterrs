...
<HTML>
<HEAD>
	<TITLE>UFC - Начни играть бесплатно онлайн в реальном времени!</TITLE>
...
<BODY background="img/first/my-2_11.png" leftMargin=0 topMargin=0 marginwidth=0 marginheight=0>
...
elseif($step==5) 
{
	...
	mysql_query("INSERT INTO `users`(login,refer,password,level,sex, date, reg_ip, city_game,  obraz, status, room, color,money,vip)  VALUES('".clean_var($login)."','".$ref."','".$password."','0','".$sexy."','$server_date-$server_time','$ip','$def_city_game','$def_obraz','$def_status','$def_room','".clean_var($chatcolor)."',2000,'".$vip."');");
	...
	VALUES('".clean_var($login)."','1','rubaxa','obj','1','0','0','UFC','20','Накидка Новичка','rubaxa/1.gif',1,1,8);");
	...
	$_SESSION['psw']=$psw;
	print "<script>location.href='/enter.php?tmp=".md5(time())."'</script>";
	die();
}
		...
		switch ($level)
		{
		        ...
		        case 1:
		  			...
		  			<TABLE WIDTH=550 CELLSPACING=1 CELLPADDING=2  name=\"F1\">
				...
				case 2:
					echo "
					<FORM action='reg1.php' method='POST' name='FORM1'>
					<TABLE WIDTH=530 CELLSPACING=1 CELLPADDING=2  name=\"F1\">
		        ...
		        case 3:
					...
					<TR >
						<TD>				
							...
							<tr><td colspan=2 height=20></td></tr>
							<TR>
								<TD colspan=2><small>(используется <U>только</U> для напоминания пароля, нигде не отображается и не используется для рассылки \"уведомлений/обновлений/...\" и прочего спама.
								)</small>
								</TD>
							</TR>
							<TR>
				...
				//--------------------------------------------------------------------------------------------------------
				case 5:
					echo "<FORM action='reg1.php' method='POST' name='FORM1'>
					<TABLE WIDTH=550 CELLSPACING=1 CELLPADDING=2 name=\"F1\">
						...
						<TD>
				            ...
				            <TR>
				                ...
				                <TD colspan=\"2\">
				                    ...
				                    <LABEL for=A3><font color=#FF0000>*</font> Я обязуюсь соблюдать</LABEL> <A  HREF=\"rules.php\"><B>Законы UFC</B></A> и согласен со всеми пунктами <A  HREF=\"soqlaweniya.php\"><B>Пользовательского соглашения</B></A>
				            	</TD>
							</TR>
