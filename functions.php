	 ...
	 <tr>
		...
		<td width=140 height=220 valign=top>
			<script>
				...
				view_item('wp/banner.gif',140,40,'UFC');\n
			</script>
		</td>
		<td width=60 height=220 valign=top>
			<script>
	 ...
	 <tr>
	    <td width=60 height=220 valign=top>
			<script>";
				if ($item_unwear['mask'])
				{
					echo "view_item_inv('items/".$item['mask']."',60,60,'".$descr['mask'].($item_unwear['helmet']?"<hr color=#504F4C size=1 width=50%>".$descr['helmet']:"")."','".$item_unwear['mask']."');";
				}
				else
				{
					echo "view_item_inv('items/".$item['helmet']."',60,60,'".$descr['helmet']."','".$item_unwear['helmet']."');";
				}
				echo "
...
function testUps($user)
{
	$exptable = array(
        				...
        				"1000000000" => array (1, 0,0,10000,0,10000000000,"Ветеран II степени",0),
				);
				/*   ups,umenie,power,money, level, next_up, status,mass*/
...
function win($team,$battle)
{
	...
	while($DATA = mysql_fetch_array($T))
	{
		...
		if(in_array($zay_type,$boylar))
		{
			...
			#new year
			#mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$WINNER_DATA['login']."', '60', 'scroll', 'magic', '1', 'UFC', '0', '1', '".(time()+30*24*3600)."');");
			#talk($WINNER_DATA["login"],"Вы нашли <b>Снежок</b>",$WINNER_DATA);
			
			#novruz
			#mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$WINNER_DATA['login']."', '262', 'scroll', 'magic', '1', 'UFC', '0', '1', '".(time()+30*24*3600)."');");
			#mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$WINNER_DATA['login']."', '262', 'scroll', 'magic', '1', 'UFC', '0', '1', '".(time()+30*24*3600)."');");
	...
	if ($zay_type==23) 
	{
		...
		if ($art_winner && $art_winner!="Исчадие Хаоса")
		{
			...
			say("toall_news","<font color=#ff0000>Внимание! <b>$art_winner</b> $str_art_winner нашел чек на АРТ. Обналичит чек на АРТ можна у <b>Торговец Вискаль</b></font>","Adminion");
		}
	}
	if ($zay_type==13)
	{
		...
		say("toall_news","<font color=#ff0000>Внимание! Битва за Башню завершилась. Победил Ханства <b>".$winner_clan_name."</b></font>","Admnion");
	}
	if ($zay_type==11)
	{
		...
		if ($svet>0)	{say("toall_news","<font color=#ff0000>Бой СВЕТ VS TЬМА закончен. <b>Победил Свет</b></font>","Adminion");}
		else if ($tma>0) {say("toall_news","<font color=#ff0000>Бой СВЕТ VS TЬМА закончен. <b>Победила Тьма</b></font>","Adminion");}
		else {say("toall_news","<font color=#ff0000>Бой СВЕТ VS TЬМА закончен. Ничья</font>","Adminion");}
	}
	if ($zay_type==66)
	{
...
function lose($team,$battle,$phrase)
{
	...
	while($DATA = mysql_fetch_array($T))
	{
		...
		{
			//new year
			//mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$LOSER_DATA['login']."', '60', 'scroll', 'magic', '1', 'UFC', '0', '1', '".(time()+30*24*3600)."');");
			//talk($LOSER_DATA["login"],"Вы нашли <b>Снежок</b>",$LOSER_DATA);
			
			//novruz
			mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$LOSER_DATA['login']."', '262', 'scroll', 'magic', '1', 'UFC', '0', '1', '".(time()+30*24*3600)."');");
