...
if(empty($tar))
{
	...
	if($_GET['do'] == 'Remind')
	{
		...
		if($SEEK)
		{
			...
			$headers .= 'From: UFC <root@old-bk.com>' . "\r\n";
	              		...
	              		if($TO_CHECK)
	              		{
							...
							else
							{
	                			...
	                			if($bank_member>0)
	                			{
	                				...
	                				{
										mysql_query("INSERT INTO inv (owner, object_id, object_type, object_razdel, msg, gift, gift_author, term) VALUES ('".$res['login']."', '419', 'flower', 'other', 'Вы являетесь Участником Новогоднего Джекпота', 1, 'UFC', '".(time()+3600*24*90)."')");
