...
switch ($mtype) 
{
	...
	case "knife9":  $zatochka = 9; $object_types='knife';break;
	...
	case "axe9":  $zatochka = 9; $object_types='axe';break;
	...
	case "sword9":  $zatochka = 9; $object_types='sword';break;
	...
	case "staff9":  $zatochka = 9; $object_types='staff';break;
...
else
{
	...
	{
		$_SESSION["message"]="Это оружие было заточено ранее!";
	}
	else
	{
		mysql_query("UPDATE inv SET is_modified=$zatochka WHERE id=".$inv['id']);
		$_SESSION["message"]="Вы удачно заточили оружие &quot".$inv['name']." +$zatochka&quot.";
		drop($spell,$DATA);
	}
}
