...
if($db["battle"] && $db["hp"] && $db["battle_opponent"]!="")
{
	...
	if($if_bot)
	{
		if ($if_bot["prototype"]=="Див")$lose_hp=3000;else $lose_hp=rand(1,15);
		$new_hp=$if_bot["hp"]-$lose_hp;
		if ($new_hp<=0){$new_hp=0;$death="<span class=date>$date</span> <b>".$target." мертв</b><br>";}
		$phrase="<span class=date>$date</span> <span class=$span>$login</span> кинул яйцом в <span class=$span2>".$target."</span>, попав в $mest_txt <span class=krit>-$lose_hp</span>. [".$new_hp."/".$if_bot["hp_all"]."]<br>$death";
	...
	else
	{
		...
		if ($new_hp<=0){$new_hp=0;$death="<span class=date>$date</span> <b>".$target." мертв</b><br>";}
		$phrase="<span class=date>$date</span> <span class=$span>$login</span> кинул яйцом в <span class=$span2>".$target."</span>, попав в $mest_txt <span class=krit>-$lose_hp</span>. [".$new_hp."/".$res["hp_all"]."]<br>$death";
