...
//-------------------------------------------------------------------
$sql2=mysql_query("SELECT `dv`,`login` FROM `users` WHERE `dv`='1' LIMIT 1");
$haves=mysql_fetch_array($sql2);
$spath=session_save_path()."/";
$userler="";
$room=$_SESSION['my_room'];
$all_online=mysql_fetch_array(mysql_query("SELECT count(*) FROM online"));
$users= mysql_query("SELECT users.login, users.id, users.dealer, users.dv, users.level, users.orden, users.admin_level, users.adminsite, users.invis, users.clan_short, users.clan, users.shut, users.travm, online.uniqPCID FROM online LEFT JOIN users on users.login=online.login WHERE online.room='".$room."' ORDER BY users.admin_level DESC,users.dealer DESC,users.clan DESC");
...
while ($onl= mysql_fetch_array($users))
{
	...
	{	
      ...
      //  @unlink($spath.$sfile);
   	...
   	if ($onl['adminsite']<2 or $onl['invis']==1)
   	{
   		
		if($onl['login']==$haves['login']){
		   
		  if($onl['dv']==1){$tr=1;}		
		                             
		}
		if($haves['login']==$my_login){
		  $tr=0; 
		  if($onl['login']!=$haves['login']){$tr=1;}
		}						 
									
									 
									
   		$userler.="usr('".$onl['login']."','".$onl['id']."','".$onl['level']."','".$onl['dealer']."','".$onl['orden']."','".$onl['admin_level']."','".$onl['clan_short']."','".$onl['clan']."','".$shut."','".$onl['travm']."','".$war."','0','".$onl['dv']."','".$tr."');\n";
	}
	else $co=$co-1;
}
...
function usr(name, id, level, dealer, align, rang, klan, klanid, slp, trv,war,att,dv,dv2)
{
	var s="";
	...
	else if (dv > 0)s+=" <a href='javascript:top.AddTo(\""+name+"\")'><font color=#FF0000>"+name+"</font></a>["+level+"]";
	...
	if (att==1){ s+=" <b><a href='#' onclick='top.frames.main.document.location=\"../main.php?chaos_attack="+id+"\";'><img src='../img/b.jpg' border=0></a></b>";}
	if (dv2 == '1'){ s+=" <b><a href='#' onclick='top.frames.main.document.location=\"../main.php?post_dv="+id+"\";'><img src='../img/b.jpg' border=0></a></b>";}
...
<td nowrap align='center'>
		 <input type='button' onclick='go();' value='Локация' style='cursor:hand' /> <input type='button' onclick='go();' value='Обновить' style='cursor:hand' /> <input type='button' onclick='gos();' value='Общий' style='cursor:hand' /><br/>
	<?echo $how?>
...
<tr>
	<td style="white-space:nowrap">
		<?
		$query=mysql_fetch_Array(mysql_query("SELECT `creator` FROM zayavka WHERE type=23"));
			...
			if ($room=="room4" && $query)echo "<script>usr('Исчадие Хаоса','".$query['creator']."','11','0','7','0','','','0','0','0','1');</script>\n";
		?>
		<script><?=$users;?></script>
	</td>
</tr>
