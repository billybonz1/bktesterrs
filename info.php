...
if(!$db || !$log_name)
{
	die('<TITLE>Произошла ошибка</TITLE><link rel=stylesheet type="text/css" href="main.css"><BODY bgcolor=#dddddd>
		...
		<hr><div align=right>© Администрация <a href="" class=us2>old-bk.com - Самый азартный проект!</a></div>');
}
?>
<html>
<head>
			    	...
			    	<tr>
			    		<td align=center>
							...
							<?
								showPlayer_inf($db);
							?>
<form action="info.php" method="get"><INPUT TYPE="text" name="log" id="log" onBlur="if (value == '') {value='Поиск персонажа';document.getElementById('log').className = 'gray';}" onFocus="if (value == 'Поиск персонажа') {value ='';document.getElementById('log').className = '';}" class="gray" value="Поиск персонажа" /> <input type="submit" value="Найти"></FORM><br/>
			    		</td>
			    	</tr>
			    </table>
			</td>
	...
	<? } ?>
		...
		<tr>
			...
			<?
				...
				if($db["vip"]>time())
				{
					echo "<img src='img/naqrada/vip.gif' border=0 alt='V.I.P Клуб UFC. \nЕще: ".convert_time($db["vip"])."'>&nbsp;";
				}
				if($db["dealer"])
				{
