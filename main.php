...
{
	$rooms=array(
	"plshop", "artshop", "smith", "magicstore", "casino", "smert_room", "dungeon", "house", "hospital", "znaxar", "bank", "sklad",
...
if($act=="go")
{
	...
	else if ($room=="municip" && in_array($level,array("arena","smith","remesl","plshop","magicstore","comok5","rep","flower","bazar"))) {$changeroom=true;}	
	else if ($level=="municip" && in_array($room,array("arena","smith","remesl","plshop","magicstore","comok5","rep","flower","bazar"))) {$changeroom=true;}	
	...
	else if (in_array($level,array("elka")) && in_array($room,array("okraina"))) 
	{
		if ($db["level"]>=4)
		{
			if (((date(n)==12 && date(j)>=1) || (date(n)==1 && date(j)<=5)) || ((date(n)==8 && date(j)<10) || (date(n)==7 && date(j)>=27)) || $db["adminsite"])$changeroom=true;
	...
	{
		if ((date(n)==12 && date(j)>=1) || (date(n)==1 && date(j)<=3))
		{	
			$changeroom=true;
		}
		else {$mess="Еще не время!";$changeroom=false;}
