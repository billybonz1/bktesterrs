...
{
	$inff="<b>Информация о ".$myinfo["login"]."</b><br>• Сила: ".$myinfo["sila"]."<br>• Ловкость: ".$myinfo["lovkost"]."<br>• Удача: ".$myinfo["udacha"]."<br>• Выносливость: ".$myinfo["power"]."<br>";
	if($myinfo["level"]>0)
	{
	...
	while($res=mysql_fetch_array($query))
	{
		...
		else
		{
			...
			if($gravirovka!=""){$desc.="<br>• Выгравирована надпись: ".$gravirovka."<br>";}
			if($add_vinoslivost){$desc.="<br>• Уровень ХП: +$add_vinoslivost";}
			if($add_mana){$desc.="<br>• Уровень маны: +$add_mana";}
			
			if($bron_head){$desc.="<br>• Броня головы: +$bron_head";}
			if($bron_arm){$desc.="<br>• Броня рук: +$bron_arm";}
			...
			if($min_attack){$desc.="<br>• Мин. урон: ".($min_attack+($is_modified?$is_modified*5:0));}
			if($max_attack){$desc.="<br>• Макс. урон: ".($max_attack+($is_modified?$is_modified*5:0));}
