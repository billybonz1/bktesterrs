...
$silver= number_format($db["silver"], 2, '.', ' ');
...
if ($_GET["stack"] && in_array($_GET["stack"],$stack))
{
	...
	{	
		...
		mysql_Query("INSERT INTO inv (owner, object_id, object_type, object_razdel, gift, gift_author, iznos, iznos_max, term) VALUES('".$login."', '".$item_id."', 'scroll', 'magic', '1', 'UFC', '0', '$count_stack', '".(time()+30*24*3600)."');");
	}
...
if($mass>$max_mass)
{
    ...
    if($db["movable"])
    {
    	...
    	talk($login,"Вы перегружены и не можете перемещаться...",$db);
    }
}
else
{
	$mass_style="color=#ffffff";
    if(!$db["movable"])
    {
        mysql_query("UPDATE users SET movable='1' WHERE login='".$login."'");
        talk($login,"Вы больше не перегружены...",$db);
    }
}
?>	
...
<tr>
	...
	<tr>
		<td valign="top" align="center" >
			...
			<tr>
				<td nowrap <?echo ($otdel=="obj"?"class='fnew'":"class='fold'");?> width="20%" valign="top" align="center" >
				<a class=us2 style="color: #ffffff" href='main.php?act=inv&otdel=obj&<?=time();?>'>Экипировка</font></a>
				</td>
				<td nowrap <?echo ($otdel=="magic"?"class='fnew'":"class='fold'");?> width="20%"  valign="top" align="center">
				<a class=us2 style="color: #ffffff" href='main.php?act=inv&otdel=magic&<?=time();?>'>Магия</a>
				</td>
				<td nowrap <?echo ($otdel=="runa"?"class='fnew'":"class='fold'");?> width="20%"  valign="top" align="center">
				<a class=us2 style="color: #ffffff" href='main.php?act=inv&otdel=runa&<?=time();?>'>Руны</a>
				</td>
				<td nowrap <?echo ($otdel=="other"?"class='fnew'":"class='fold'");?> width="20%"  valign="top" align="center">
				<a class=us2 style="color: #ffffff" href='main.php?act=inv&otdel=other&<?=time();?>'>Открытки</a>
				</td>
				<td nowrap <?echo ($otdel=="thing"?"class='fnew'":"class='fold'");?> width="20%"  valign="top" align="center">
				<a class=us2 style="color: #ffffff" href='main.php?act=inv&otdel=thing&<?=time();?>'>Ресурсы</a>
				</td>
				<td nowrap <?echo ($otdel=="flud"?"class='fnew'":"class='fold'");?> width="20%"  valign="top" align="center">
	...
	<tr>
		<td colspan="5" class='fnew' align=center valign="center" height=20>
			<? echo "<b>Рюкзак (<font $mass_style>масса: ".round($mass,2)."</font>/".$max_mass.", Всего вещей: $al)</b>";?>
		</td>
	</tr>
	<tr>
		<td  align=center>
		...
		else if($otdel=="magic")
		{
			...
			while($DAT = mysql_fetch_array($S))
			{
		        ...
		        echo "</td>";
		        echo "<td valign=top>
		        <b>".$name."</b> ".($artovka?"<img src='img/icon/artefakt.gif' border=0 alt=\"АРТЕФАКТ\">":"").($gift?"&nbsp;<img src='img/icon/gift.gif' border=0 alt=\"Подарок от ".$gift_author."\">":"")." (Масса: ".$DAT["mass"].")<br>";
		...
		echo "<tr class='fnew' height=10><td colspan=2></td></tr>";
?>
	</table>
   </td>
  </tr>
