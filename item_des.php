...
function show_item($db,$item_Array)
{
	...
	$str.=(($item_Array['min_udacha'])?"• ".(($item_Array['min_udacha'] > ($db['udacha']+$effect['add_udacha']) && count($db)>0)?"<font color=red>":"")."Интуиция: {$item_Array['min_udacha']}</font><BR>":"");
	...
	$str.=(($item_Array['add_lovkost']<0)?"• Ловкость: {$item_Array['add_lovkost']}<BR>":"");
	$str.=(($item_Array['add_udacha']>0)?"• Интуиция: +{$item_Array['add_udacha']}<BR>":"");
	$str.=(($item_Array['add_udacha']<0)?"• Интуиция: {$item_Array['add_udacha']}<BR>":"");
	...
	if($item_Array['magic']==1){$stl="<font color=#FF0000><b>Огня</b></font>";;}
	if($item_Array['magic']==2){$stl="<font color=#000099><b>Воды</b></font>";;}
	if($item_Array['magic']==3){$stl="<font color=#00CC00><b>Земли</b></font>";;}
	if($item_Array['magic']==4){$stl="<font color=#00CCFF><b>Воздуха</b></font>";}	
	if($item_Array['magic']==5){$stl="<font color=#FFFFFF><b>Света</b></font>";}	
	if($item_Array['magic']==6){$stl="<font color=#000000><b>Тьмы</b></font>";}	
	if($item_Array['magic']==7){$stl="<font color=#CCCCCC><b>Серой магии</b></font>";}	
	$str.=(($item_Array['magic'])?"<br><font color=brown>Наложено заклятие:</font> ".$stl."":"");	
