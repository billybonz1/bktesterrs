...
function findwear(title, script, name, mtype) {
	var s;
	s='<form action="'+script+'" method=POST ><table border=0 width=100% cellspacing="0" cellpadding="2"><tr><td colspan=2>'+
	'Oea?eoa iacaaiea:<small></TD></TR><TR><TD width=50% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME="'+name+'" id="'+name+'"  value=""></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT=""></TD></TR></TABLE></FORM>';
	s = crtmagic(mtype, title, s);
...
function countitems(title, script, name, mtype) 
{
	...
	'<table border=0 width=100% cellspacing="0" cellpadding="2">'+
		'<tr><td width=30% nowrap>Eiee?anoai:</TD><td style="padding-left:5" nowrap><INPUT  TYPE="text" NAME="item_count" maxlength=1 size=5></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT=""></TD></TR></TABLE></FORM>';
	s = crtmagic(mtype, title, s);
...
function AddCount(title, script, name, mtype) 
{
	...
	s='<form action="'+script+'" method=POST name=slform>'+
	'<table border=0 width=100% cellspacing="0" cellpadding="2">'+
		'<tr><td colspan=2>Eiee?anoai (oo.)</td></tr>'+
...
function lot(title, script, name, mtype) 
{
	...
	'<b>'+name+'</b> </TD></TR><TR><TD valign=center>Oea?eoa oaio: <INPUT  TYPE="text" NAME="lot_price" maxlength=5 size=5> <INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT=""></TD></TR></TABLE></FORM>';
	s = crtmagic(mtype, title, s);
...
// Caaieiaie, iacaaiea ne?eioa, eiy iiey n eiaeiii, eiaei ii oiie?aie?, oei ioi?ieaiey, oei n oaeu? eee aac
function bMag(title, script, name, defaultlogin, mtype, btype, text){
    ...
    '<td colspan=2>'+text+'<br>Oea?eoa eiaei ia?niia?a:<small><BR>(ii?ii uaeeioou ii eiaeio a ?aoa)</TD>'+
    ...
    else if(btype==2)
    {
		s='<form action="'+script+'" method=POST name=slform>'+
		'<table border=0 width=100% cellspacing="0" cellpadding="2"><tr>'+
    	'<td colspan=2>'+text+'<br>Oea?aeoa aaoo oaio:</TD>'+
    	'</TR><TR><TD width=50% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME="'+name+'" id="'+name+'"  value="'+defaultlogin+'"></TD>'+
    ...
    {
		s='<form action="'+script+'" method=POST name=slform><table border=0 width=100% cellspacing="0" cellpadding="2"><tr><td colspan=2>'+
	    text+'</TD></TR><TR><TD width=50% align=left><INPUT class=new TYPE="submit" name="tmpname423" value="Aa" style="width:70%"></TD>'+
	    '<TD width=50% align=right><INPUT class=new type=button style="width:70%" value="Iao" onclick="closehint3();"></TD></TR></TABLE></FORM>';
    }
...
function lock(title, script, name, defaultlogin, mtype) {
	var s;
	s='<form action="'+script+'" method=POST name=action><table border=0 width=100% cellspacing="0" cellpadding="2"><tr><td colspan=2>'+
	'Oea?eoa ia?ieu:</TD></TR><TR><TD width=80% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME=pass1></TD><TD width=20%></TD></TR><TR><TD colspan=2>'+
	'Iiaoi?eoa ia?ieu:</TD></TR><TR><TD width=80% align=right style="padding-left:5"><INPUT style="width: 100%" TYPE="text" NAME="pass2" value=""></TD><TD width=20%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT=""></TD></TR></TABLE></FORM>';
	s = crtmagic(mtype, title, s);
...
function dialogconfirm(title, script, text, mtype) {
	...
	'<tr><td width=50% align=left><input type="submit" name="tmpname423" id="tmpname423" value="Aa"  class="new" style="width:70%"></td><td width=50% align=right><input class="new" type=button style="width:70%" value="Iao" onclick="closehint3();"></td></tr>'+
	'</table></form>';
	s = crtmagic(mtype, title, s);
	document.getElementById("hint4").innerHTML = s;
	document.getElementById("hint4").style.visibility = "visible";
...
function Daily_Gift(title, script, text, mtype) {
	...
	'<table border=0 width=100% cellspacing="0" cellpadding="2">'+
	'<tr><td colspan=2>'+text+'</td></tr>'+
	'<tr><td width=50% align=left><input type="submit" name="yes" id="yes" value="Aa"  class="new" style="width:70%"></td><td width=50% align=right><input class="new" type="submit" name="no" value="Iao" style="width:70%"></td></tr>'+
	'</table></form>';
	s = crtmagic(mtype, title, s);
...
function confirmetion(title, id, script, text, mtype) {
	...
	text+'</TD></TR><TR><TD width=50% align=left><INPUT class=new TYPE="text" name="char_id" value="'+id+'" style="display:none;"><INPUT class=new TYPE="submit" name="tmpname423" value="Aa" style="width:70%"></TD><TD width=50% align=right><INPUT class=new type=button style="width:70%" value="Iao" onclick="closehint3();"></TD></TR></FORM></TABLE>';
...
function magicklogin(title, script, magickname, n, defaultlogin, extparam, mtype) {
	...
	'Oea?eoa eiaei ia?niia?a:<small><BR>(ii?ii uaeeioou ii eiaeio a ?aoa)</TD></TR><TR><TD style="padding-left:5" width=50% align=right><INPUT TYPE="text" NAME="param" value="'+defaultlogin+'" style="width: 100%"></TD><TD width=50%><INPUT type=image SRC="#IMGSRC#" WIDTH="27" HEIGHT="20" BORDER=0 ALT=""></TD></TR>';
...
while (body.indexOf('#IMGSRC#')>=0) body = body.replace('#IMGSRC#', 'http://UFC/img/dmagic/'+names[mtype*10]+'_30.gif');
var s='<table width="270" border="0" align="center" cellpadding="0" cellspacing="0">'+
	...
	'<tr><td>'+
		...
		'<tr>'+
		'<td width="'+names[mtype*10+1]+'" align="left"><img src="http://UFC/img/dmagic/b'+names[mtype*10]+'_03.gif" width="'+names[mtype*10+1]+'" height="'+names[mtype*10+2]+'"></td>'+
		'<td width="100%" align="right" background="http://UFC/img/dmagic/b'+names[mtype*10]+'_05.gif"></td>'+
		'<td width="'+names[mtype*10+3]+'" align="right"><img src="http://UFC/img/dmagic/b'+names[mtype*10]+'_07.gif" width="'+names[mtype*10+3]+'" height="'+names[mtype*10+2]+'"></td>'+
		'</tr>'+
		'</table></td>'+
	'</tr>'+
	'<tr><td>'+
		...
		'<tr>'+
			...
			'<td width="5" background="http://UFC/img/dmagic/b'+names[mtype*10]+'_17.gif">&nbsp;</td>'+
			'<td width="100%">'+
			'<table width="100%" border="0" cellspacing="0" cellpadding="0">'+
			...
			'</td><td width=20 align=right valign=top style="cursor: pointer" onclick="closehint3();" style=\'filter:Gray()\' onmouseover="this.filters.Gray.Enabled=false" onmouseout="this.filters.Gray.Enabled=true"><IMG src="http://UFC/img/dmagic/clear.gif" width=13 height=13>&nbsp;</td></table>'+
			'</td></tr>'+
			'<tr>'+
				'<td align="center" bgcolor="#'+colors[mtype*2+1]+'">'+body+
			'</tr>'+
			'</table></td>'+
			'<td width="5" background="http://UFC/img/dmagic/b'+names[mtype*10]+'_19.gif">&nbsp;</td>'+
	...
	'<tr><td>'+
		'<table width="100%"  border="0" cellpadding="0" cellspacing="0">'+
		'<tr>'+
			'<td width="'+names[mtype*10+4]+'" align="left"><img src="http://UFC/img/dmagic/b'+names[mtype*10]+'_27.gif" width="'+names[mtype*10+4]+'" height="'+names[mtype*10+6]+'"></td>'+
			'<td width="100%" align="right" background="http://UFC/img/dmagic/b'+names[mtype*10]+'_29.gif"></td>'+
			'<td width="'+names[mtype*10+5]+'" align="right"><img src="http://UFC/img/dmagic/b'+names[mtype*10]+'_31.gif" width="'+names[mtype*10+5]+'" height="'+names[mtype*10+6]+'"></td>'+
		'</tr>'+
		'</table></td>'+
	'</tr>'+
	'</table></td>'+
